...
function renderPublicSite() {
  ...
  if (slug.startsWith('blog/')) return renderBlogPostPage(slug.split('/')[1]);
...
function renderBlogPostPage(postSlug) {
  const post = (state.site.pluginData?.blog?.posts || []).find((item) => item.slug === postSlug || item.id === postSlug);
  if (!post) return `<main><section class="block empty-state">Blog post not found.</section></main>`;
  return `<main>
    <section class="site-hero-strip"><p>${escapeHtml(state.site.settings.siteName)}</p><div>${getPublicNavigation().map((item) => `<a href="${escapeHtml(item.url)}">${escapeHtml(item.label)}</a>`).join('')}</div></section>
    <article class="block blog-post-page"><p class="eyebrow">${escapeHtml(post.category || 'Blog')}</p><h1>${escapeHtml(post.title)}</h1><p class="post-meta">${escapeHtml(post.author)} · ${escapeHtml(post.status)}</p>${renderRichText(post.content || '')}</article>
  </main>`;
}
